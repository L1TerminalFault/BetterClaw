...
export default function Message({
  ...
  const handleCopy = async () => {
    ...
    setCopying(true);
    const copyBtn = document.querySelector("#copyable:hover") as HTMLElement;
  ...
  if (message.role === "user")
    return (
      <div className="flex flex-col group gap-1.5 w-full items-end">
        <div
          id="copyable"
          className="items-end flex max-w-2/3 flex-col gap-1.5"
        >
          ...
          <div className="flex w-full h-full justify-between items-center">
            ...
            >
              {copying ? (
                <MdDone className="size-3.5" />
              ) : (
                <LuCopy className="text-white transition-all group-hover:opacity-100 opacity-0 size-3.5" />
              )}
            </div>
    ...
    return (
      <div className="flex flex-col group gap-1.5 w-full items-start pr-9">
        <div
          id="copyable"
          className="flex flex-col gap-1.5 items-start max-w-full"
        >
